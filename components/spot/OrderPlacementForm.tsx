...
import { placeSpotOrderTx } from "@/lib/utils/spot/transactions";
...
export default function OrderPlacementForm({
  ...
  const onSubmit = async (values: OrderPlacementFormValues) => {
    setIsSubmitting(true);
    setSubmitError(null);
    setTxStatus(null);

    const quantity = values.totalUnits ?? 0;
    const amount = values.totalAmount ?? 0;
    const price = values.orderType === "market" ? null : (values.entryPrice ?? 66500);

    // Open wallet modal
    setWalletTxAmount(amount.toString());
    setShowWalletModal(true);
    setWalletStep("idle");
    setCurrentTxHash(undefined);

    const result = await placeSpotOrderTx(
      {
        orderType: values.orderType as "limit" | "market" | "trigger",
        side: values.orderSide,
        pair: "BTC/USDT",
        price,
        quantity,
        amount,
        loop: values.loopEnabled ? `${values.noOfLoops ?? "∞"}` : undefined,
      },
    ...
    if (result.success) {
      if (result.position) {
        ...
        setSpotTrade({ activePositions: [result.position, ...prevPositions] });
      }
      if (result.openOrder) {
        const prevOpenOrders = getSpotTrade((s) => s.openOrders);
        setSpotTrade({ openOrders: [result.openOrder, ...prevOpenOrders] });

        // Simulate limit order fill after random delay (5-15s)
        if (values.orderType === "limit") {
          const fillDelay = 5000 + Math.random() * 10000;
          setTimeout(() => {
            const currentOpenOrders = useSpotTradeStore.getState().openOrders;
            const orderToFill = currentOpenOrders.find((o: any) => o.id === result.openOrder!.id);
            if (orderToFill) {
              const filledPosition = mapOrderToActivePosition(values, price ?? 66500);
              const updatedOpen = currentOpenOrders.filter((o: any) => o.id !== result.openOrder!.id);
              const currentPositions = useSpotTradeStore.getState().activePositions;
              useSpotTradeStore.getState().set({
                openOrders: updatedOpen,
                activePositions: [filledPosition, ...currentPositions],
              });
            }
          }, fillDelay);
        }
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center justify-between ">
          ...
          <span className="flex items-center gap-1 text-[10px] leading-[15px] font-medium text-[#919191]">
            <Image
              ...
              src="/icons/info.svg"
            />
            MB: {marginBalance.toFixed(2)} USDT
          </span>
        </div>
      ...
      {!userAddress ? (
        ...
        <Button
          text={isSubmitting
            ? (txStatus === "approving" ? "Approving..." : txStatus === "confirming" ? "Confirming..." : "Processing...")
            : "Place Order"
          }
          size="small"
          type="solid"
      ...
      <WalletTxModal
        ...
        step={walletStep}
        action={`${orderSide === "buy" ? "Buy" : "Sell"} BTC/USDT`}
        asset="USDT"
        amount={walletTxAmount}
